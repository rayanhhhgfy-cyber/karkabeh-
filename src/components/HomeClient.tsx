    ...
    <main className="min-h-screen bg-white" dir={language === "ar" ? "rtl" : "ltr"}>
      ...
      <header className="sticky top-0 z-40 bg-white/80 backdrop-blur-md border-b border-gray-100 px-6 py-4 flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] text-gold uppercase tracking-[0.2em] font-semibold">{t(language, "luxuryCollection")}</span>
        </div>
        <div className="flex items-center gap-3">
          <button 
            onClick={() => setLanguage(language === "en" ? "ar" : "en")}
            className="px-3 py-1 rounded-full bg-gray-100 text-gray-700 text-xs font-bold uppercase hover:bg-gray-200 transition-all"
          >
            {language === "en" ? "عربي" : "EN"}
          </button>
          <Link
        ...
        <div className="absolute inset-0 flex flex-col justify-center items-center text-center px-6">
          <div className="animate-in fade-in slide-in-from-bottom-10 duration-1000 ease-out">
            ...
            <p className="text-white text-lg md:text-2xl font-medium tracking-wide drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)] max-w-3xl leading-relaxed">
              {t(language, "heroDescLine1")}
              <br className="hidden md:block" />
              {t(language, "heroDescLine2")}
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-white px-6 py-20">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center mb-16">
            <h3 className="text-xs font-black uppercase tracking-[0.4em] text-gold mb-4 text-center">{t(language, "collectionYear")}</h3>
            <div className="w-12 h-[1px] bg-gold mb-6"></div>
            <h2 className="text-4xl font-bold text-maroon italic text-center tracking-tight">{t(language, "curatedMasterpieces")}</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {products.length === 0 ? (
              ...
              products.map((product) => {
                const thumbnail = product.images?.[0]?.image || "/assets/bag-maroon.png";
                return (
                  <Link 
                    key={product.id}
                  ...
                  >
                    <div className="relative aspect-[4/5] bg-gray-50 rounded-3xl overflow-hidden shadow-sm transition-all duration-500 group-hover:shadow-2xl group-hover:-translate-y-2 border border-gray-100">
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-8">
                        <span className="text-gold text-[10px] font-black uppercase tracking-widest mb-1">{t(language, "viewDetails")}</span>
                        <h4 className="text-white text-2xl font-bold italic tracking-tight">{language === "ar" ? product.nameAr : product.nameEn}</h4>
                        <p className="text-gold/80 text-sm font-bold mt-1">{language === "ar" ? product.nameEn : product.nameAr}</p>
                      </div>
                      {product.hasBadge && (
                    ...
                    <div className="mt-6 text-center">
                      <h4 className="text-lg font-bold text-gray-900 group-hover:text-maroon transition-colors">{language === "ar" ? product.nameAr : product.nameEn}</h4>
                    </div>
                  </Link>
                );
              })
            ...
            >
              <div className="relative aspect-[4/5] bg-maroon rounded-3xl overflow-hidden shadow-sm transition-all duration-500 group-hover:shadow-2xl group-hover:-translate-y-2 flex flex-col items-center justify-center p-10 text-center border-4 border-gold/20">
                ...
                <h4 className="text-white text-3xl font-bold italic tracking-tight mb-4">{t(language, "bespokeChoice")}</h4>
                <p className="text-white/60 text-sm leading-relaxed mb-4">
                  {t(language, "bespokeDesc")}
                </p>
      ...
      <footer className="py-12 px-6 bg-gray-50 border-t border-gray-100 text-center">
        ...
        <div className="mt-8 flex justify-center gap-4">
          ...
          <p className="text-[10px] text-gold uppercase tracking-widest font-bold">{t(language, "verifiedLuxury")}</p>
          <div className="w-8 h-[1px] bg-gold self-center"></div>
        </div>
      </footer>
    </main>
